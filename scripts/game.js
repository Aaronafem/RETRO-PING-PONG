  ...
  startButton.addEventListener("click", function () {
    if (!playerName) {
      ...
      window.location.href =
        "game.html?playerName=" +
        encodeURIComponent(playerName) +
        "&selectedTexture=" +
        encodeURIComponent(selectedTexture);
    }
  });
...
// Adding event listener for the space bar key
document.addEventListener("keyup", function (event) {
  if (event.key === " ") {
    // Toggle the pause state
    isPaused = !isPaused;
    // Show/hide the flashing text based on the pause state
...
function setup() {
  ...
  document.getElementById("winnerBoard").innerHTML = "GAME OF " + maxScore;
  ...
  ballTexture = new THREE.ImageUtils.loadTexture(
    `./img/ball/ball${selectedTexture}.jpg`
  );
...
function draw() {
  ...
  //Check to see is the game is paused
  controllerInput();
  if (!isPaused) {
    //controllerInput();
    ballPhysics();
...
function playerPaddleMovement() {
  ...
  if ((Key.isDown(Key.A) && !speedPressed) || (leftPressed && !speedPressed)) {
    ...
    // else we don't move and stretch the paddlead
  ...
  //is pressed, we speed up
    ...
    (Key.isDown(Key.A) && (speedPressed || Key.isDown(Key.S))) ||
  ...
  //speed right
    ...
    (Key.isDown(Key.D) && (speedPressed || Key.isDown(Key.S))) ||
    (rightPressed && speedPressed)
  ) {
    // if paddle is not touching the side of table
    // we move
